...
class EchoShieldApp:
    ...
    def create_analysis_tab(self, parent):
        ...
        # Student selection for frequency matching
        student_frame = tk.Frame(parent, bg='#34495e')
        student_frame.pack(fill='x', padx=20, pady=10)
        
        tk.Label(student_frame, text="Match Against Student:", font=('Arial', 12), 
                fg='#ecf0f1', bg='#34495e').pack(side='left')
        
        self.student_var = tk.StringVar()
        self.student_combo = ttk.Combobox(student_frame, textvariable=self.student_var, width=30)
        self.student_combo.pack(side='left', padx=10)
        
        tk.Button(student_frame, text="Refresh Students", font=('Arial', 10), bg='#27ae60', fg='white',
                 command=self.refresh_student_list).pack(side='right')
    ...
    def analyze_recording(self):
        """Analyze the selected recording for frequency signatures"""
        if not self.selected_audio_file:
            messagebox.showerror("Error", "Please select an audio file first")
            return
        
        if not self.student_var.get():
            messagebox.showerror("Error", "Please select a student to match against")
            return
        
        try:
            # Extract student info
            student_info = self.student_var.get()
            student_id = student_info.split('(')[1].split(')')[0]
            
            # Get student's frequencies from database
            records = self.db.get_all_frequency_records()
            ...
            for record in records:
                if record[0] == student_id:
                    ...
                    target_frequencies = json.loads(record[2])
                    break
            
            if not target_frequencies:
                messagebox.showerror("Error", "No frequency data found for selected student")
                return
            ...
            self.results_text.insert(tk.END, f"Analyzing against: {student_name} ({student_id})\n\n")
            self.results_text.update()
            
            # Detect frequencies
            detection_results, overall_confidence = self.audio_analyzer.detect_frequencies(
                audio_data, target_frequencies
            )
            ...
            detected_count = 0
            for freq, result in detection_results.items():
                if result['detected']:
                    detected_count += 1
                    status = "✓ DETECTED"
                    color = "green"
            ...
            self.results_text.insert(tk.END, f"Overall Confidence: {overall_confidence:.1f}%\n\n")
            
            if detected_count >= 3:  # At least 3 out of 4 frequencies detected
                verdict = "RECORDING LIKELY PROTECTED"
                self.results_text.insert(tk.END, f"VERDICT: {verdict}\n")
                self.results_text.insert(tk.END, "This recording appears to contain the expected frequency signatures.\n")
                ...
                self.results_text.insert(tk.END, f"VERDICT: {verdict}\n")
                self.results_text.insert(tk.END, "Recording does not appear to contain expected frequency signatures.\n")
